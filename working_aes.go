...
func submitForm(w http.ResponseWriter, r *http.Request) {
	...
	// formData.Passw, _ = RSAEncrypt(publicKey, []byte(formData.Passw))
	// Convert the encrypted data to a base64-encoded string
	x1, _ := RSAEncrypt(publicKey, []byte(formData.Passw))
	formData.Passw = base64.StdEncoding.EncodeToString(x1)

	// Process the form data as needed
	fmt.Printf("Received data: %+v\n", formData)
	fmt.Println("The discord ID is ", formData.Discord)
	// enc_disc, err := GetAESEncrypted(formData.Discord, key, iv)
	enc_disc, err := RSAEncrypt(publicKey, []byte(formData.Discord))
